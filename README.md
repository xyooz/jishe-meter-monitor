...
- 抄表完成后直接查询最新状态并写入 Cloudflare D1
- Web 看板支持“立即抄读”和“刷新最新数据”两种操作
- GitHub Actions 保留为手动备用抄读链路，不再承担定时任务
- Cloudflare Workers 提供查询、历史、统计、月历和手动抄读 API
...
这样即使无人打开网页，也会由 Cloudflare 自动完成远程抄读并持续积累约 5 分钟粒度的历史数据。

网页还提供两种手动操作：

- **立即抄读**：真实调用 Reading 接口，完成一次远程抄读，随后查询最新状态并写入 D1。
- **刷新最新数据**：只读取集云社后台当前已有状态，不额外触发一次物理/远程抄表。

## GitHub Actions 的定位

`.github/workflows/test.yml` 目前只保留 `workflow_dispatch` 手动触发，用于 Cloudflare 异常时的备用抄读和排障验证。
...
src/index.js                   看板、查询、统计等 HTTP API
src/worker.js                  Worker 总入口；Cron 自动抄读与手动抄读
...
Workers Logs / Invocation Logs 也已在 Wrangler 中开启，可在 Cloudflare 控制台中搜索：
